...
<form action="{{ route('profiles.store') }}" method="POST">
    ...
    <p>
        ...
        <select name="router_id" required>
            ...
            @foreach($routers as $id => $name)
                <option value="{{ $id }}" {{ old('router_id') == $id ? 'selected' : '' }}>
                    {{ $name }}
                </option>
            @endforeach
        </select>
    ...
    <p>
        ...
        <input type="text" name="rate_limit" value="{{ old('rate_limit') }}">
    </p>

    <p>
        <label>Time limit (minutes):</label><br>
        <input type="number" name="time_limit_minutes" value="{{ old('time_limit_minutes') }}" min="0">
    </p>

    <p>
        <label>Data limit (MB):</label><br>
        <input type="number" name="data_limit_mb" value="{{ old('data_limit_mb') }}" min="0">
    </p>
